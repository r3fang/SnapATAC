...
#' @param genes A GRanges object contains the gene annotation
...
#' 
#' @return Returns a snap objects contains cell-by-gene matrix
#' 
...
#' @export
createGmat <- function(obj, genes, ncell.chunk, do.par, num.cores) {
  UseMethod("createGmat", obj);
}

#' @export
createGmat.default <- function(
	obj,
	genes,
	ncell.chunk=20,
	do.par=TRUE,
	...
	ind.ls = mclapply(id.ls, function(x){
		...
		ov = findOverlaps(reads.gr, genes);
		idx = match(
	...
	obj@gmat = 	sparseMatrix(i=ind[,1], 
							 j =ind[,2], 
							 x=ind[,3], 
							 dims=c(length(barcodes), length(genes))
							 );
	colnames(obj@gmat) = genes$name;
	rm(ind);
	gc();
	return(obj);		
}
